...
def rank_result(resume_summary, jd_by_id_dict, model_name, top_n, version, is_resume=True):
    send_log(f">>>Starting to assess, match and rank your resume with filtered job descriptions by Gemini API")
    rank_result_llm = match_and_rank(resume_summary, jd_by_id_dict,model_name, top_n, version, is_resume)
    rank_result = extract_json_from_response(rank_result_llm)
    return rank_result

def match_and_rank(resume_summary, job_summaries, model_name, top_n=5, version='version1', is_resume=True):
    if is_resume:
        prompt = f"""
        ...
        Rank the top {top_n} matches of jd based on qualification and suitability to the resume.
        ...
        match_data = f"""
            ...
            dictionary of job descriptions:{job_summaries}
        """
    else:
        prompt = f"""
        Given a job description summary and dictionary of {len(job_summaries)} resumes where keys are unique resume IDs and values are summaries. Rank the top {top_n} matches of resume based on qualification and suitability to the job description summary.
        ...
        """
        match_data = f"""
            job description summary:{resume_summary}
            dictionary of resumes:{job_summaries}
        """       
